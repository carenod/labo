...
hs <- makeParamSet( 
  ...
  makeIntegerParam("max_bin",          lower= 2L   , upper= 250L)
...
PARAM$experimento  <- "002_DAC2_cloud" # DAC porque es mio, 2 por la segunda competencia
...
PARAM$hyperparametertuning$semilla_azar  <- 987353
#Aqui poner la propia semilla, PUEDE ser distinta a la de trainingstrategy
...
EstimarGanancia_lightgbm  <- function( x )
{
  ...
  ganancias <- c()
  seed_set <- c(987353, 9, 19, 1993, 5783, 2022,
                353789, 7, 91, 3919, 7358, 2202)
  ...
  prom_ganancias <- mean(ganancias)

  param_completo$num_iterations <- modelocv$best_iter  #asigno el mejor num_iterations
  param_completo["early_stopping_rounds"]  <- NULL     #elimino de la lista el componente  "early_stopping_rounds"
  
  #Voy registrando la importancia de variables
  if( prom_ganancias >  GLOBAL_gananciamax )
  {
    GLOBAL_gananciamax  <<- prom_ganancias
  ...
  attr(prom_ganancias ,"extras" )  <- list("num_iterations"= modelocv$best_iter)  #esta es la forma de devolver un parametro extra
  
  #logueo 
  xx  <- param_completo
  xx$ganancia  <- prom_ganancias   #le agrego la ganancia
  xx$iteracion <- GLOBAL_iteracion
  loguear( xx, arch= klog )
  
  return( prom_ganancias )
}
...
vars_for_pca <- names(dataset)[sapply(dataset, is.numeric)]
vars_for_pca = vars_for_pca[! vars_for_pca %in% c("foto_mes", "numero_de_cliente", "clase01")]

pca_03 <- PCA(dataset[foto_mes==202103, ..vars_for_pca], 
              scale.unit = TRUE, 
              graph = FALSE)

# fviz_eig(pca_03, addlabels = TRUE, ylim = c(0, 10))

#Agrego las dim del PCA al dataset
dataset[foto_mes==202103, dim1 := pca_03$ind$coord['Dim.1']]
dataset[foto_mes==202103, dim2 := pca_03$ind$coord['Dim.2']]
dataset[foto_mes==202103, dim3 := pca_03$ind$coord['Dim.3']]
dataset[foto_mes==202103, dim4 := pca_03$ind$coord['Dim.4']]
dataset[foto_mes==202103, dim5 := pca_03$ind$coord['Dim.5']]

# Para 202105 quizas lo puedo hacer after data drifting solving
# pca_05 <- PCA(dataset[foto_mes==202105, ..vars_for_pca], 
#               scale.unit = TRUE, 
#               graph = FALSE)
# dataset[foto_mes==202105, dim1 := pca_05$ind$coord['Dim.1']]
# dataset[foto_mes==202105, dim2 := pca_05$ind$coord['Dim.2']]
# dataset[foto_mes==202105, dim3 := pca_05$ind$coord['Dim.3']]
# dataset[foto_mes==202105, dim4 := pca_05$ind$coord['Dim.4']]
# dataset[foto_mes==202105, dim5 := pca_05$ind$coord['Dim.5']]
...
# 
# PARAM <- list()
# PARAM$experimento  <- "001_DAC2_Kaggle"
# 
# PARAM$input$dataset       <- "./datasets/competencia2_2022.csv.gz"
# PARAM$input$training      <- c( 202103 )
