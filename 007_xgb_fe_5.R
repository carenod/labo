...
setwd("D:/FCEN/Materias/DataMining/Materias/DMEF/")
PARAM <- list()
PARAM$experimento  <- "007_DAC2_FE_xgb_5"
...
param_fe <- list(
  ...
  learning_rate = 0.05,
  max_depth = 30, # <--- IMPORTANTE CAMBIAR
  num_parallel_tree = 5000, # <--- IMPORTANTE CAMBIAR
  subsample = 0.6,
...
new_features <- xgb.create.features(model = xgb_model, data.matrix(dataset[foto_mes == 202103 | foto_mes == 202105]))
