...
PARAM$experimento  <- "003_DAC_reparar_EC" # DAC porque es mio, 2 por la segunda competencia

PARAM$input$dataset       <- "./exp/03_DAC_reparar/dataset_EC.csv.gz"
PARAM$input$training      <- c( 202004, 202005, 202006 )

PARAM$trainingstrategy$undersampling  <-  0.5   # un undersampling de 0.1  toma solo el 10% de los CONTINUA
...
# mejores hp:
# learning_rate 0.0235381000467388
# num_iterarions 2004
# feature_fraction 0.970520869205697
# min_data_in_leaf 6
# num_leaves 311

PARAM <- list()
PARAM$experimento  <- "003_DAC_reparar_EC_predicion"

PARAM$input$dataset       <- "./exp/03_DAC_reparar/dataset_EC.csv.gz"
PARAM$input$training      <- c( 202004, 202005, 202006 )
PARAM$input$future        <- c( 202008 )

PARAM$finalmodel$max_bin           <-     31
PARAM$finalmodel$learning_rate     <-      0.0235381000467388
PARAM$finalmodel$num_iterations    <-    2004
PARAM$finalmodel$num_leaves        <-   311
PARAM$finalmodel$min_data_in_leaf  <-   6
PARAM$finalmodel$feature_fraction  <-     0.970520869205697
...
dim(subset(tb_entrega, tb_entrega$prob > 0.000001))

tb_entrega[ foto_mes == 202008, ]

dim(tb_entrega)

tb_entrega <- tb_entrega[dataset[ foto_mes == 202008, .(numero_de_cliente, clase_ternaria)], on = .(numero_de_cliente), nomatch = 0]

ggplot(tb_entrega, aes(x = prob, color=clase_ternaria)) +
  geom_density() +
  xlim(0.001, 1)
setorder( tb_entrega, -prob )
tb_entrega[1:9000, ganancia := ifelse(clase_ternaria == 'BAJA+2', 78000, -2000)]

sum(tb_entrega[1:9000, ganancia])
